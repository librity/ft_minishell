...
/*   Updated: 2022/09/24 18:59:02 by lpaulo-m         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
MU_TEST(get_int_tst)
{
	...
	c()->last_exit = "0";
	mu_assert_int_eq(0, last_exit_int());

	c()->last_exit = "42";
	mu_assert_int_eq(42, last_exit_int());

	c()->last_exit = "4242";
	mu_assert_int_eq(4242, last_exit_int());
...
{
	mu_check(true == set_last_exit(-5));
	mu_assert_string_eq("-5", c()->last_exit);
